...
keys = [
    ...
    Key([my_mod, "control"],           "Return",                   lazy.layout.flip()),

    # resize windows
    Key([my_mod, "control"],           "h",                        lazy.layout.shrink()),
    Key([my_mod, "control"],           "l",                        lazy.layout.grow()),
    Key([my_mod, "control"],           "m",                        lazy.layout.maximize()),
    Key([my_mod, "control"],           "r",                        lazy.layout.reset()),
    ...
    # screenshots
    Key( [],                           "Print",                    lazy.spawn(expanduser("sc"))),
    Key( [],                           "Print",                    lazy.spawn(expanduser("~/scripts/sc -s"))),
    Key( [my_mod],                     "Print",                    lazy.spawn(expanduser("sc -r"))),
    Key( [my_mod, "shift"],            "Print",                    lazy.spawn(expanduser("~/scripts/sc -rs"))),
    ...
    Key([my_mod],                   "XF86AudioRaiseVolume",       lazy.spawn("amixer sset Master 15%+")),
    Key([],                         "XF86AudioLowerVolume",       lazy.spawn("amixer sset Master 5%-")),
    Key([my_mod],                   "XF86AudioLowerVolume",       lazy.spawn("amixer sset Master 15%-")),
    ...
    KeyChord([my_mod], "d", [
        ...
        Key([], 'q',   lazy.group['scratchpad'].dropdown_toggle('qshell')),
    ])
]
...
# layouts
my_layout = {"border_focus" : my_colors["white"],
             "border_width" : 2,
             "margin" : 0,
...
my_sctarchpads = ScratchPad("scratchpad", [
    ...
    DropDown("htop", "xterm -e htop",
             x=0.05, y=0.05, width=0.9, height=0.9, opacity=1,
             on_focus_lost_hide=True),
    DropDown("qshell", "xterm -hold -e qshell",
             x=0.05, y=0.05, width=0.9, height=0.9, opacity=1,
             on_focus_lost_hide=True)])
        ...
        top=Bar(
            [
                widget.GroupBox(
                    ...
                    this_current_screen_border=my_colors["white"],
                    active=my_colors["yellow"],
                    inactive=my_colors["white"],
                    disable_drag=True,
                ),
                widget.TextBox(text="|",),
                ...
                widget.ThermalSensor(threshold=50, foreground_alert=my_colors["red"],foreground=my_colors["white"]),
                widget.TextBox(text="|",),
