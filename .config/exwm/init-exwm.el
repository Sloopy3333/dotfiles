...
(defun sam/exwm-volume-ctl (opt)
  (progn
    ...
    (notifications-notify
     :title (concat "Volume is "
                    (shell-command-to-string "amixer sget Master | grep -o -m 1 '[[:digit:]]*%' | tr -d '%'")
                    "\t"
                    (shell-command-to-string "amixer sget Master | grep -o -m 1 '\[[a-z]*\]' |  tr -d '%[]'"))
     :timeout 2000))
  )
...
(defun sam/exwm-backlight-ctl (opt)
  ...
  (sit-for 1)
  (notifications-notify
   :title (concat "Backlight is "
                  (format "%-1.1s" (shell-command-to-string "xbacklight -get")))
   :timeout 2000)
  )
...
(use-package exwm
  ...
  ;; volume control
  (exwm-input-set-key (kbd "<XF86AudioRaiseVolume>") (lambda () (interactive) (sam/exwm-volume-ctl "5%+")jkj))
